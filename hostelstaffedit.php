...
if(isset($_REQUEST['update_id']))
{
	try
	{
		...
		$editselectqry = "SELECT * FROM man_t WHERE id='$id'"; 
...
if(isset($_REQUEST['btn_update']))
{
	try
	{
		...
		$path="hostelstaffphoto/".$image_file; 
		
		$directory="hostelstaffphoto/"; 
		...
		if(!isset($errorMsg))
		{
			$updateqry = "UPDATE man_t SET name='$name', post='$post', image='$image_file' WHERE id='$id'"; 
			$updateresult = mysqli_query($conn, $updateqry);
			 
			if($updateresult)
			{
				$updateMsg="File Updated Successfully.......";	
				header("refresh:3;hostelstaffmanage.php");	
		}
	}
}
	catch(PDOException $e)
	...
	<div class="container">
		...
		?>   
			...
			<form method="post" class="form-horizontal" enctype="multipart/form-data">
				...
				<div class="col-sm-6">
				<input type="text" name="post" class="form-control" value="<?php echo $post; ?>" required/>
				</div>
				</div>	
				...
				<p><img src="hostelstaffphoto/<?php echo $image; ?>" height="100" width="100" /></p>
				</div>
				</div>
				...
				<a href="hostelstaffmanage.php" class="btn btn-danger">Cancel</a>
				</div>
				</div>
