...
if(isset($_REQUEST['btn_insert']))
{
	try
	{
		...
		if(!isset($errorMsg))
		{
			...
			$post = $post;
			$image = $image_file;
			$insertqry = "INSERT INTO man_t (`image`, `name`, `post`) VALUES ('$image', '$name', '$post')";
			$insertresult = mysqli_query($conn, $insertqry); 
		
			if(!$insertqry)
			{
				$insertMsg="Staff Member Upload Failed"; 
				header("refresh:3;addhostelstaff.php"); 
			} else {
			$insertMsg="Staff Member Upload Successfully........"; 
				header("refresh:3;headofficestaff.php");
		}
	}
	}
