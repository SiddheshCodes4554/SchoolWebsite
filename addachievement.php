	...
	try
	{
		...
		{	
			if(!file_exists($path)) 
			{
				if($size < 5000000)
				{
					move_uploaded_file($temp, "achievementsphoto/" .$image_file); 
				}
				else
				{
		...
		if(!isset($errorMsg))
		{
			...
			if(!$insertqry)
			{
				...
				header("refresh:3;addachievement.php"); 
			} else {
			$insertMsg="Acheivement Upload Successfully........"; 
		...
		?>   	
			<form method="post" class="form-horizontal" enctype="multipart/form-data">
				...
				<input type="file" name="txt_file" class="form-control" />
                    <small style="color:red;">Use Photo Of Size 346 x 195</small>
				</div>
				</div>
